const { next, nextDev, _app, nextIndex, nextConfig, tailwindCSS, tailwindConfig, postCssConfig, tailwindDeps, postInstallMessage } = require("./dependencies.js");
...
function generateNextProd(pwd, tailwind, ts, pName) {
    ...
    execSync(`npm i ${nextDep}`, { cwd: pwd }, (err, stdout, stderr) => {
        if (err) {
            console.log(err);
        }
    });
    ...
    if(tailwind) {
        ...
        execSync(`npm i -D ${tailwindDep}`, { cwd: pwd }, (err, stdout, stderr) => {
            if (err) {
                console.log(err);
            }
        });
        ...
        fs.writeFileSync(`${pwd}\\tailwind.config.js`, tailwindConfig)
